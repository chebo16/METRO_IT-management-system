...
public final class UserEditServlet extends HttpServlet {
    ...
    private void prepareEditForm(
        ...
        request.setAttribute("selectedRole", roleValue);
    }
    ...
    private void validateCurrentAccountRoleChange(
        ...
        boolean editingCurrentAccount =
                sessionUser.getId().equals(existingUser.getId());

        boolean roleWasChanged =
                existingUser.getRole() != selectedRole;

        if (editingCurrentAccount && roleWasChanged) {
            throw new ValidationException(
    ...
    private boolean hasNewPassword(String rawPassword) {
